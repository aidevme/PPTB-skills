  ...
  <div class="card">
    ...
    <div class="example-group">
      ...
      <div class="button-group">
        <button
          @click="showNotification('Success!', 'Operation completed successfully', 'success')"
          class="btn btn-success"
        >
          Show Success
        </button>
        <button
          @click="showNotification('Information', 'This is an informational message', 'info')"
          class="btn btn-info"
        >
          Show Info
        </button>
        ...
        <button @click="showNotification('Error', 'An error has occurred', 'error')" class="btn btn-error">
          Show Error
        </button>
        <button @click="showLoading" class="btn btn-primary">Show Loading</button>
      </div>
    </div>
...
const showLoading = async () => {
  try {
    await window.toolboxAPI.utils.showLoading("Loading for 3 seconds please wait...");
    // Simulate some async operation
    setTimeout(async () => {
      await window.toolboxAPI.utils.hideLoading();
      emit("log", "Loading indicator hidden", "info");
    }, 3000);
  } catch (error) {
    emit("log", `Error showing loading indicator: ${(error as Error).message}`, "error");
  }
};
