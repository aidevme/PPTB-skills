...
**Prerequisites**: Familiarity with the [Agent Skills specification](/miscellaneous/agent-skills/specification/), which defines the `SKILL.md` file format, frontmatter fields, and directory conventions.
...
See the [specification](/miscellaneous/agent-skills/specification/) for the full set of frontmatter fields and their constraints.
...
> **Note:** The [specification](/miscellaneous/agent-skills/specification/) defines strict constraints on the `name` field (matching the parent directory, character set, max length). The lenient approach above deliberately relaxes these to improve compatibility with skills authored for other clients.
