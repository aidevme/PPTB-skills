...
- **Keep it concise.** A few sentences to a short paragraph is usually right — long enough to cover the skill's scope, short enough that it doesn't bloat the agent's context across many skills. The [specification](/miscellaneous/agent-skills/specification/#description-field) enforces a hard limit of 1024 characters.
...
> **Tip:** If your agent client supports it, you can stop a run early once the outcome is clear — the agent either consulted the skill or started working without it. This can significantly reduce the time and cost of running the full eval set.
...
2. Verify the description is under the [1024-character limit](/miscellaneous/agent-skills/specification/#description-field).
...
Once your skill triggers reliably, you'll want to evaluate whether it produces good outputs. See [Evaluating skill output quality](/miscellaneous/agent-skills/evaluating-skills/) for how to set up test cases, grade results, and iterate.
